...
public class ForecastActivity extends FragmentActivity {
    ...
    private class GetWeatherTask extends AsyncTask<Double, Void, JSONArray> {
    	...
    	protected JSONArray doInBackground(Double... params) {
			...
			try {
				URL url = new URL(String.format("http://api.openweathermap.org/data/2.1/find/city?lat=%.6f&lon=%.6f&cnt=1", latitude, longitude));
  				JSONObject json = getJson(url);
				JSONObject station = json.getJSONArray("list").getJSONObject(0);
				int station_id = station.getInt("id");
  				url = new URL(String.format("http://api.openweathermap.org/data/2.2/forecast/city/%s?mode=daily_compact&units=metric", station_id));
  				json = getJson(url);
        ...
        public Fragment getItem(int position) {
            ...
            try {
            	if (weatherForecast != null) {
            		// TODO This is unset until the async task completes
            		args.putString(ForecastFragment.ARG_FORECAST, weatherForecast.get(position).toString());
            	}
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
    ...
    public static class ForecastFragment extends Fragment {
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
            ...
            View rootView = inflater.inflate(R.layout.fragment_forecast_dummy, container, false);
